...
async function errorHandled<type = Response>(
  ...
  try {
    ...
    if (response instanceof Response && !response.ok) {
      ...
      const defaultMsg: ApiErr = {
        error: `StatusText: ${response.statusText} message: ${bod}`,
    ...
    const unknownErr: ApiErr = {
      error: `An Unknown error occurred ${JSON.stringify(error)}`,
    };
    if (!(error instanceof Error))
      return [null, new Error(JSON.stringify(unknownErr))];
    let err = error;
    if (!isJsonObject(error.message))
      err = new Error(JSON.stringify({ error: JSON.stringify(err.message) }));
    return [null, err];
  }
}
